...
import { updateUserDetails } from "../../../app/slices/userSlice"; // Import action
...
function Step2({ currentStep, setCurrentStep }) {
  ...
  const userDetails = useSelector((state) => state.user); // Lấy dữ liệu từ Redux
  ...
  const formik = useFormik({
    ...
    validationSchema,
    enableReinitialize: true, // Đảm bảo formik cập nhật lại dữ liệu khi Redux thay đổi
    onSubmit: (values) => {
      dispatch(updateUserDetails(values)); // Cập nhật Redux store
